...
import com.example.tetris.presenters.PresenterCompletableObserver;
import com.example.tetris.presenters.PresenterObserver;
...
public class TetrisGameModel implements GameModel {
    ...
    private void generateUpcomingBricks(){
        ...
        for(int i = 1; i < UPCOMING_AREA_SIZE; i++) {
            for (int j = 1; j < UPCOMING_AREA_SIZE; j++) {
    ...
    private boolean isNextMerged(){
        for(Point fallingPoint: mFallingPoints) {
            if(fallingPoint.y + 1 >= 0 && (fallingPoint.y == PLAYING_AREA_HEIGHT - 1 || getPlayingPoint(fallingPoint.x, fallingPoint.y + 1).isStablePoint())){
                return true;
            }
        }
        return false;
    ...
    private void updatePlayingPoint(Point point){
        if(point.x >= 0 && point.x < PLAYING_AREA_WIDTH && point.y >= 0 && point.y < PLAYING_AREA_HEIGHT){
    ...
    public void turn(GameTurn turn) {
        ...
        switch (turn) {
            case LEFT:
                ...
                canTurn = true;
                for (Point fallingPoint: mFallingPoints) {
                    if (fallingPoint.y >= 0 && (fallingPoint.x == 0 || getPlayingPoint(fallingPoint.x - 1, fallingPoint.y).isStablePoint())){
                        canTurn = false;
                        break;
                    }
                }
            ...
            case RIGHT:
                ...
                canTurn = true;
                for (Point fallingPoint: mFallingPoints) {
                    if (fallingPoint.y >= 0 && (fallingPoint.x == PLAYING_AREA_WIDTH - 1 || getPlayingPoint(fallingPoint.x + 1, fallingPoint.y).isStablePoint())){
                        canTurn = false;
                        break;
                    }
                }
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++){
                ...
                if (point == null) {
                    final int tmx = j + x;
                    final int tmy = j + y;
                    point = mFallingPoints.stream().filter(p -> p.x == tmx && p.y == tmy).findFirst().orElse(new Point(j + x, i + y));
                }
                if (point.isStablePoint() && getPlayingPoint(x + size - 1, y + j).isFallingPoint) {
                    canRotate = false;
                    break;
                }
                points[i][j] = new Point(x + size - i, y + j, point.type, point.isFallingPoint);
            }
            if (!canRotate) {
                break;
            }
